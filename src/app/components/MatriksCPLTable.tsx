...
export default function MatriksCPLTable({
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (!cplRes.ok) {
        throw new Error(`Gagal mengambil data CPL: ${cplRes.status} ${cplRes.statusText}`);
      }
      if (!mkRes.ok) {
        throw new Error(`Gagal mengambil data mata kuliah: ${mkRes.status} ${mkRes.statusText}`);
      }
  ...
  const toggleCPL = (cplKode: string) => {
    setCollapsedCPL(prev => 
      prev.includes(cplKode) ? prev.filter(k => k !== cplKode) : [...prev, cplKode]
    );
  };
    ...
    <div className={`${isFullscreen ? 'fixed inset-0 z-50 bg-white p-8 overflow-auto' : ''}`}>
        ...
        <div className={`overflow-x-auto ${maxHeight} overflow-y-auto`} ref={tableRef}>
          {loading ? (
            <div className="p-12 text-center">
              <Loader2 className="animate-spin inline text-indigo-600 mb-3" size={40} strokeWidth={2.5} />
              <p className="text-sm text-gray-700 font-semibold">Memuat matriks CPL...</p>
            </div>
          ) : matakuliahList.length === 0 || cplList.length === 0 ? (
            <div className="p-12 text-center">
              <AlertCircle className="inline text-gray-400 mb-3" size={40} />
              <p className="text-sm text-gray-700 font-semibold mb-2">Tidak ada data</p>
              <p className="text-xs text-gray-500">
                {cplList.length === 0 ? 'Belum ada CPL yang terdaftar.' : 'Belum ada mata kuliah yang terdaftar.'}
              </p>
            </div>
          ) : (
            <table className="min-w-full text-[11px] border-collapse">
              <thead className="sticky top-0 z-20">
                <tr>
                  <th 
                    rowSpan={2} 
                    className="border-2 border-white/20 px-4 py-4 text-center font-bold text-white sticky left-0 bg-gradient-to-r from-indigo-600 to-blue-600 z-30 text-xs" 
                  ...
                  <th 
                    rowSpan={2} 
                    className="border-2 border-white/20 px-4 py-3 text-center font-bold text-white sticky bg-gradient-to-r from-indigo-600 to-blue-600 z-30 text-xs shadow-[4px_0_12px_-2px_rgba(79,70,229,0.3)]" 
                    style={{
                      minWidth: mkColWidth, 
                      width: mkColWidth,
                      left: mkColLeft
                  ...
                  {sortedCPL.map((cpl, cplIdx) => {
                    ...
                    return (
                      <th 
                        key={cpl.id} 
                        colSpan={ikCount || 1}
                        className={`border-2 border-white/30 px-2 py-4 text-center font-bold text-white text-xs bg-gradient-to-br ${design.primary} ${isFirstOfGroup ? 'border-l-4 border-l-white' : ''}`}
                      >
                        <div className="flex flex-col items-center gap-1.5">
                          ...
                          {!compactMode && (
                            <div className="flex items-center gap-1 text-[9px] opacity-80">
                              ...
                              <button onClick={() => toggleCPL(cpl.kode_cpl)} className="ml-1 hover:bg-white/20 p-0.5 rounded">
                                {collapsedCPL.includes(cpl.kode_cpl) ? <ChevronRight size={10} /> : <ChevronDown size={10} />}
                              </button>
                            </div>
                          )}
                        </div>
                      ...
                      return (
                        ...
                        >
                          <div className="flex flex-col items-center gap-0.5">
                            <span className="text-xs font-extrabold">{ikNumber}</span>
                            {!compactMode && (
                              <>
                                <span className="text-[7px] opacity-60 uppercase">{cpl.kode_cpl}</span>
              ...
              <tbody>
                {sortedSemesters.map((semester) => {
                  ...
                  return mkInSemester.map((mk, mkIdx) => (
                    <tr key={mk.id} className={`group transition-colors ${mkIdx % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'} hover:bg-indigo-50/50`}>
                      {mkIdx === 0 && (
                        <td 
                          rowSpan={mkInSemester.length} 
                          className="border-2 border-gray-300 px-3 py-3 text-center font-extrabold text-base text-indigo-900 bg-gradient-to-br from-gray-50 to-gray-100 sticky left-0 z-10"
                        >
                          {semester === 0 ? '-' : semester}
                        </td>
                      )}
                      ...
                      >
                        <div className="font-bold text-[11px] text-gray-900 leading-tight mb-1">{mk.nama}</div>
                        <div className="flex items-center gap-2">
                          <span className="text-[9px] font-bold text-indigo-700 bg-indigo-50 px-2 py-0.5 rounded border border-indigo-200">{mk.kode_mk}</span>
                          {!compactMode && <span className="text-[8px] text-gray-500">{mk.sks} SKS</span>}
                        </div>
                      </td>
                      {sortedCPL.map(cpl => {
                        if (collapsedCPL.includes(cpl.kode_cpl)) return null;
                          ...
                          return (
                            ...
                            >
                              <div className="flex items-center justify-center h-8">
                                {currentState === 'checked' && <CheckCircle className={`w-5 h-5 ${design.text}`} strokeWidth={2.5} />}
                                {currentState === 'saving' && <Loader2 className="w-4 h-4 text-yellow-600 animate-spin" />}
                                {currentState === 'error' && <AlertCircle className="w-4 h-4 text-red-600" />}
                              </div>
      ...
      {compactMode && !loading && matakuliahList.length > 0 && (
        <div className="mt-3 flex items-center justify-between text-xs text-gray-600 bg-gray-50 px-4 py-2 rounded-lg">
          ...
          <span className="font-bold text-indigo-700">{totalMapping} Mapping</span>
        </div>
      )}
    </div>
  );
