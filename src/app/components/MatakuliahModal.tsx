        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-1">
              ...
              <input type="text" value={kodeMK} onChange={(e) => setKodeMK(e.target.value)} className="mt-1 block w-full px-3 py-2 border rounded-md" required />
            </div>
            <div className="col-span-1">
              <label className="block text-sm font-medium text-gray-700">SKS</label>
              <input type="number" value={sks} onChange={(e) => setSks(e.target.value)} className="mt-1 block w-full px-3 py-2 border rounded-md" required min="0" />
            </div>
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700">Nama Mata Kuliah</label>
            <input type="text" value={nama} onChange={(e) => setNama(e.target.value)} className="mt-1 block w-full px-3 py-2 border rounded-md" required />
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700">Semester</label>
              <input type="number" value={semester} onChange={(e) => setSemester(e.target.value)} className="mt-1 block w-full px-3 py-2 border rounded-md" placeholder="1-8" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700">Sifat</label>
              <select value={sifat} onChange={(e) => setSifat(e.target.value)} className="mt-1 block w-full px-3 py-2 border rounded-md">
          ...
          <div className="border-t pt-4 mt-2">
            <label className="block text-sm font-bold text-gray-700 mb-2">Dukung CPL (Pilih Minimal Satu)</label>
            <div className="max-h-40 overflow-y-auto border rounded-md p-2 bg-gray-50 grid gap-2">
                {cplList.length === 0 ? (
                    <p className="text-xs text-gray-500 italic text-center">Belum ada data CPL di kurikulum ini.</p>
                ) : (
                    cplList.map((cpl) => (
                        <label key={cpl.id} className="flex items-start gap-2 cursor-pointer hover:bg-gray-100 p-1 rounded">
                            ...
                            <div>
                                ...
                                <span className="block text-xs text-gray-600 leading-tight">{cpl.deskripsi}</span>
                            </div>
                        </label>
                    ))
                )}
            </div>
            <p className="text-xs text-gray-500 mt-1">Checklist CPL yang dibebankan pada mata kuliah ini.</p>
          </div>

          <div className="flex justify-end gap-3 pt-4">
            <button type="button" onClick={onClose} className="px-4 py-2 bg-gray-200 text-gray-700 rounded hover:bg-gray-300">Batal</button>
