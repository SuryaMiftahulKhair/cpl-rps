...
export default function IkManagementModal({
  isOpen,
  onClose,
  onSuccess,
  kurikulumId,
}: IkManagementModalProps) {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(`/api/kurikulum/${kurikulumId}/ik/${id}`, {
        method: "DELETE",
      });
      if (!res.ok) throw new Error(await parseApiError(res));
      loadData();
      ...
      <div className="bg-white p-6 rounded-xl shadow-xl w-full max-w-4xl my-8">
        ...
        {isFormOpen ? (
          <form
            ...
            className="mb-6 p-5 border border-indigo-100 rounded-xl bg-indigo-50/30 space-y-4">
            <h3 className="font-semibold text-indigo-900">
              {selectedIk ? "Edit" : "Tambah"} Indikator
            </h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setFormData((p) => ({ ...p, cpl_id: e.target.value }))
                  }
                  className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-black"
                  required>
                  {cplList.length === 0 && (
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData((p) => ({ ...p, kode_ik: e.target.value }))
                  }
              ...
              <div className="md:col-span-2">
                ...
                <textarea
                  ...
                  className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm h-20 text-black"
                  required
                />
              </div>
            </div>
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <button
                type="submit"
                disabled={submitting || cplList.length === 0}
          ...
          <button
            ...
            className="mb-6 flex items-center gap-2 bg-teal-600 text-white px-4 py-2.5 rounded-lg text-sm font-medium hover:bg-teal-700 shadow-sm transition-all">
        ...
        <div className="border border-gray-200 rounded-xl overflow-hidden shadow-sm">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-5 py-3 text-left font-semibold text-xs text-gray-600 uppercase">
                  Kode IK
                </th>
                <th className="px-5 py-3 text-left font-semibold text-xs text-gray-600 uppercase">
                  Induk CPL
                ...
                <th className="w-24 px-5 py-3 text-center font-semibold text-xs text-gray-600 uppercase">
                  Aksi
                </th>
              </tr>
            </thead>
