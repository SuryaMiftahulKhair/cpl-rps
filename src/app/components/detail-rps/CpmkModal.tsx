        ...
        <form
            ...
            {!availableIks || availableIks.length === 0 ? (
              ...
              <div className="max-h-48 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                {availableIks.map((ik: any) => (
                  <label
                    key={ik.id}
                    className="flex items-start gap-3 p-3 bg-white hover:bg-indigo-50 rounded-lg cursor-pointer border border-slate-200 transition-all group">
                    <input
                      type="radio"
                      {...register("ik_id", {
                        required: "Pilih salah satu IK",
                      })}
                      value={ik.id}
                    ...
                    <div className="text-[11px] leading-relaxed w-full">
                      ...
                      <span className="text-gray-600 group-hover:text-gray-900 block">
                        {ik.deskripsi}
                      </span>
                    </div>
                  </label>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t sticky bottom-0 bg-white">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 text-sm text-gray-500 font-semibold hover:bg-gray-100 rounded-lg">
              Batal
            </button>
            <button
              type="submit"
              disabled={isSaving}
              className="px-4 py-2 bg-teal-600 text-white rounded-lg flex items-center gap-2 disabled:opacity-50 text-sm font-bold shadow-md hover:bg-teal-700">
              {isSaving ? (
