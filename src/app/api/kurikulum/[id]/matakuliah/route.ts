...
export async function POST(
  ...
  try {
    ...
    if (!checkKurikulum) {
        return NextResponse.json({ error: "Anda tidak memiliki akses ke kurikulum prodi ini!" }, { status: 403 });
    }
    ...
    if (err.code === 'P2002') {
        return NextResponse.json({ error: "Kode Mata Kuliah sudah ada di database!" }, { status: 400 });
    }
    return NextResponse.json({ success: false, error: err.message }, { status: 500 });
  }
}